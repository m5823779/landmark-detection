...
class COCODataset(Dataset):
    ...
    def __init__(self,
                 root_path="./datasets/monitor", data_version="train", is_train=True, use_gt_bboxes=True, bbox_path="",
        ...
        self.annotation_path = os.path.join(
            self.root_path, 'annotations', 'keypoints_%s.json' % self.data_version
        )
        ...
        self.nof_joints = 4
        self.nof_joints_half_body = 2  # 8
        self.flip_pairs = [[0, 1], [1, 2], [2, 3], [3, 0]]
        self.upper_body_ids = [1, 2]  #right
        self.lower_body_ids = [0, 3]  # left
        ...
        self.transform = transforms.Compose([
            ...
            transforms.Normalize(mean=[0.512, 0.570, 0.594], std=[0.103, 0.094, 0.100])
        ])
        ...
        for imgId in tqdm(self.imgIds):
            ...
            for obj in objs:
                ...
                self.data.append({
                    'imgId': imgId,
                    'annId': obj['id'],
                    'imgPath': os.path.join(self.root_path, self.data_version, '%05d.jpg' % imgId),
                    'center': center,
                    'scale': scale,
                    'joints': joints,
